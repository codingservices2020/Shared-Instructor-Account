...
async def generate_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    codes.append(code)
    save_codes(codes)
    await update.message.reply_text(f"Generated Subscription code: {code}")
...
async def handle_code_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_code"):
        ...
        if code_input in codes:
            ...
            await update.message.reply_text(f"<b>🔰SUBSCRIPTION CODE VERIFIED!🔰</b>\n\n Please enter your full name:", parse_mode="HTML",)
        else:
    ...
    if context.user_data.get("verified"):
        ...
        subscription_data[user_id] = {"name": user_name, "expiry": expiry_date}
        save_subscription_data()
        logger.info(f"User {user_id} ({user_name}) plan expires on {expiry_date}.")

        day = expiry_date.strftime("%Y-%m-%d")
        time = expiry_date.strftime("%H:%M")

        try:
            invite_link = await context.bot.create_chat_invite_link(PRIVATE_CHANNEL_ID, member_limit=1)
            await update.message.reply_text(
                f"🚀 Here is your premium member invite link:\n\n{invite_link.invite_link}\n"
                f"<b>(Valid for one-time use)</b>\n\n"
                f"✅ After joining this channel, type command /start to access the instructor account.\n\n"
                f"<b>🌐 Your plan will expire on {day} at {time}.</b>",
                parse_mode="HTML"
            )
        except Exception as e:
...
async def show_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_list = "\n".join([
        f"👤 <a href='tg://user?id={chat_id}'>{details['name']}</a> "
        f"(Expiry: {details['expiry'].strftime('%Y-%m-%d %H:%M')})"
        for chat_id, details in subscription_data.items()
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        sent_message = await update.message.reply_text(
            "<b>🔰You are already a premium member!🔰</b>" if is_premium else f"<b>🔰You are not a premium member!🔰</b>\n\n"
                                                                   f" To use this bot, you must first purchase a subscription. "
                                                                   f"Please click on the button below to contact Admin and make the payment.",
...
/generate_code - (Admin only) Generate a 6-digit code
